...
std::vector<std::vector<Point2D>> mapProcess::findGoalLineGroups(costmap_2d::Costmap2D* costmap, const std::vector<geometry_msgs::PoseStamped>& local_plan, std::pair<double,double> global_goal){
  ...
  // std::cout<<farther_goal_list_index<<std::endl;
  // for (auto x=0; x<small_width; x++){
  //   for (auto y=0; y<small_height; y++){
  //     std::cout<<map_obstacle_labeled[x][y];
  //   }
  //   std::cout<<std::endl;
  // }
  ...
  for (auto iter=all_goal_line_points.begin(); iter!=all_goal_line_points.end(); iter++){
    if (map_obstacle_labeled[iter->x-x_min][iter->y-y_min] == farther_goal_list_index){
      ...
      if(!one_goal_line.empty()){
        // 长度大于7的goal line才会被考虑，因为它会被收缩边界
...
void mapProcess::initialize(costmap_2d::Costmap2D* costmap, const teb_local_planner::PoseSE2& start, const teb_local_planner::PoseSE2& goal, const std::vector<geometry_msgs::PoseStamped>& local_plan, const std::vector<std::vector<double>>& dynamic_obstacle, std::pair<double,double> global_goal){
  ...
  // 进一步忽略ignore_dis的obstacle
  int ignore_dis = 4;
  for (int y=shrink_dis_; y<=height_large_-shrink_dis_-1; y++){
    for (int x=shrink_dis_; x<=width_large_-shrink_dis_-1; x++){
      if (x<shrink_dis_+ignore_dis || x>width_large_-shrink_dis_-ignore_dis-1 || y<shrink_dis_+ignore_dis || y>height_large_-shrink_dis_-ignore_dis-1)
        map_vector_.push_back((char)0);
      else
        map_vector_.push_back(data[ x + y*width_large_ ]);
  ...
  for (auto p:dynamic_obstacle_){
    int mx, my;
    std::pair<double,double> robot2goal = {start.position().x()-global_goal.first-p[0],start.position().y()-global_goal.first-p[1]};
    std::pair<double,double> robot2human = {start.position().x()-p[0],start.position().y()-p[1]};
    ...
    if (robot2human_project_on_robot2goal<-0.5) continue;   // 如果人落后于机器人1米以上，则忽略
    // ROS_INFO("    retained");
    world2map(mx,my,p[0],p[1]);
    for (int dx = -length_in_map_; dx<=length_in_map_; dx++){
      for (int dy=-length_in_map_; dy<=length_in_map_; dy++){
        if (mx+dx>=ignore_dis && mx+dx<=width_-ignore_dis && my+dy>=ignore_dis && my+dy<=height_-ignore_dis && std::hypot(dx,dy)<=length_in_map_){
          map_vector_[mx+dx+(my+dy)*width_] = 100;
          map_is_dynamic_[mx][my] = true;
        }
      }
    }
  }
  // set the borderlines of [map_vector_] as 0
  ...
  // convert [map_cv_] with the unit being 0 or 1
  ...
  std::vector<int8_t> map_vector_without_pedestrians;
  for (int y=0; y<=height_large_-1; y++){
    for (int x=0; x<=width_large_-1; x++){
      map_vector_without_pedestrians.push_back(data[ x + y*width_large_ ]);
    }
  }
  // convert [map_cv_without_dynamic_obs_] with the unit being 0 or 1
  std::vector<uint8_t> map_int_without_pedestrians;
  for (int i=0; i<map_vector_without_pedestrians.size(); i++){
    auto p2 = map_vector_without_pedestrians[i];
    if (p2 != (char)0)
      map_int_without_pedestrians.push_back('d');
    else
      map_int_without_pedestrians.push_back((uint8_t)p2);
  }
...
std::map<int, std::vector<Point2D>> mapProcess::cornerIdentify(){
  std::map<int,std::vector<Point2D>> temp_corner_list;    // 保存每个obstacle group的corner point
  // 为每个障碍物 利用闭运算 求得其corner list: temp_corner_list
  for (auto ID:label_list_){
    // 对每个obstacle group求它的矩形边界
    int min_x=width_, max_x=0, min_y=height_, max_y=0;
    for (auto p:border_list_[ID]){
      if (p.x<min_x)
        min_x = p.x;
      if (p.y<min_y)
        min_y = p.y;
    ...
    if (knl_y%2==0)
      knl_y++;
    cv::Mat kernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(knl_x, knl_y));
    cv::Mat resultMat;
  ...
  //       // 原有点是boder
  //       if (resultMat.at<uchar>(i,j)==1)
  //         corner_list_[ID].push_back(Point2D(local_map_x, local_map_y));
  //     }
  //   }
  // }

    // 若原有的border点 仍是border点，则其进化为corner点
    std::vector<Point2D> delta_set = {Point2D(-1,-1),Point2D(-1,0),Point2D(-1,1),Point2D(0,-1),Point2D(0,1),Point2D(1,-1),Point2D(1,0),Point2D(1,1)};
    for (int i=length_x; i<length_x*2; i++){
      for (int j=length_y; j<length_y*2;j++){
        int local_map_x = i+min_x-length_x;
        ...
        //   corner_list_[ID].push_back(Point2D(local_map_x, local_map_y));
        if (resultMat.at<uchar>(i,j)==1 && map_border_labeled_[local_map_x][local_map_y]==ID){
          // 膨胀后依然是border
          int empty_count = 0;
          for (auto delta:delta_set){
            if (resultMat.at<uchar>(i+delta.x,j+delta.y)==0){
              empty_count++;
            }
          }
          // 转角点
          if (empty_count>=4)
            temp_corner_list[ID].push_back(Point2D(local_map_x, local_map_y));
        }
      }
    }
  }

  corner_before_intersection_list_ = temp_corner_list;

  // 初步赋值map_corner_labeled_ 
  std::map<int,std::vector<Point2D>> temp_ordered_corner_list;
  for (auto corner:temp_corner_list){
    for (auto p:corner.second){
      map_corner_labeled_[p.x][p.y] = corner.first;
    }
  }
  // 把现有的corner按照boder的顺序进行排列得到temp_ordered_corner_list
  for (auto iter_o=border_list_.begin(); iter_o!=border_list_.end();iter_o++){
    ...
    for (auto iter_i=iter_o->second.begin(); iter_i!=iter_o->second.end();iter_i++){
      if (map_corner_labeled_[iter_i->x][iter_i->y] == label){
        temp_ordered_corner_list[label].push_back(Point2D(iter_i->x, iter_i->y));
      }
    }
  }

  // 考虑两个obstacle group的交叉:
  // 将所有corner投射到一个map_fill_corner上
  // 根据 任意obstacle groupA的相邻两corner点的直接连线BC 如果cover到某corner点D，则D必定来自其他障碍物：
  //   对一个障碍物的相邻corner点，用breshman连接中间（不包含端点），如果这个连线中有其他障碍物，则这里得有交叉
  std::vector<std::vector<bool>> map_filll_corner = std::vector<std::vector<bool>> (width_, std::vector<bool>(height_, false));
  for (auto corner_list:temp_ordered_corner_list){
    int ID = corner_list.first;
    int length = corner_list.second.size();
    for (int i=0; i<length; i++){
      std::vector<Point2D> line_vec= Bresenham(corner_list.second[i], corner_list.second[getMod(i+1,length)]);
      for (auto p:line_vec){
        if (map_border_labeled_[p.x][p.y]!=ID && map_border_labeled_[p.x][p.y]!=0){
          map_filll_corner[corner_list.second[i].x][corner_list.second[i].y] = true;
          map_filll_corner[corner_list.second[getMod(i+1,length)].x][corner_list.second[getMod(i+1,length)].y] = true;
          break;
        }
      }
    }
  ...
  // 赋值corner_list_，包含正常的corner点，和boundary fill而成的
  for (auto iter_o=border_list_.begin(); iter_o!=border_list_.end();iter_o++){
    int label = iter_o->first;
    int last_fill_index = -1;
    for (int i=0; i<iter_o->second.size(); i++){
      Point2D poi = iter_o->second[i];
      // 到了corner点：
      if (map_corner_labeled_[poi.x][poi.y] == label){
        // 已经有last_fill_index了 + 当前是fill点： 则填充前序的boundary + 当前corer点 + 更新last_fill_index
        if (last_fill_index!=-1 && map_filll_corner[poi.x][poi.y]){
          for (int j=last_fill_index+1; j<=i;j++)
            corner_list_[label].push_back(iter_o->second[j]);
          last_fill_index = i;
        }
        // 没有last_fill_index了 + 当前是fill点： 则填充当前corner + 更新last_fill_index
        else if (last_fill_index==-1 && map_filll_corner[poi.x][poi.y]){
          corner_list_[label].push_back(Point2D(poi.x, poi.y));
          last_fill_index = i;
        }
        // 无论是否有last_fill_index + 当前不是fill点： 则填充当前corner + 抹除last_fill_index
        else if (!map_filll_corner[poi.x][poi.y]){
          corner_list_[label].push_back(Point2D(poi.x, poi.y));
          last_fill_index = -1;
        }
      }
  ...
  // 二次赋值map_corner_labeled_
  for (auto corner_list:corner_list_){
    int ID = corner_list.first;
    for (auto p:corner_list.second){
      map_corner_labeled_[p.x][p.y] = ID;
    }
  }

  // 重塑border_list_，如果前后两个corner点可以直接连接，则直接脸
  border_origin_list_ = border_list_;
  std::map<int, std::vector<Point2D>> replace_border_list;
  for(auto temp:border_list_){
    int ID = temp.first;    
    std::vector<Point2D> temp_list;
    int last_index = -1;
    for (int i=0; i<temp.second.size(); i++){
      if (map_corner_labeled_[temp.second[i].x][temp.second[i].y]==0 && last_index==-1)
        temp_list.push_back(temp.second[i]);
      else if (map_corner_labeled_[temp.second[i].x][temp.second[i].y]!=0 && last_index==-1)
        last_index = i;
      else if (map_corner_labeled_[temp.second[i].x][temp.second[i].y]!=0 && last_index!=-1 || i==temp.second.size()-1){
        auto vec_line = Bresenham(temp.second[last_index], temp.second[i]);
        // 比较长度
        int dis1=0, dis2=0;
        for (int j=0; j<vec_line.size()-1; j++)
          dis1 += abs(vec_line[j+1].x-vec_line[j].x) + abs(vec_line[j+1].y-vec_line[j].y);
        for (int j=last_index; j<i; j++)
          dis2 += abs(temp.second[last_index+1].x - temp.second[last_index].x) + abs(temp.second[last_index+1].y - temp.second[last_index].y);
        if(checkCollisionOnlyObs(vec_line) || abs(dis1-dis2)<=1){
          // 把vec_line添加到temp_list最后
          temp_list.insert(temp_list.end(), temp.second.begin()+last_index, temp.second.begin()+i);          
        }
        else{
          temp_list.insert(temp_list.end(), vec_line.begin(), vec_line.end()-1);
        }
        last_index = i;
      }
      else
    ...
    temp_list.push_back(temp.second.back());
    replace_border_list[ID] = temp_list;
  }
  border_list_ = replace_border_list;
  // 重塑map_border_labeled_
  map_border_labeled_ = std::vector<std::vector<int>> (width_, std::vector<int>(height_, 0));
  for (auto temp:border_list_){
    int ID = temp.first;
    for (auto p:temp.second)
      map_border_labeled_[p.x][p.y] = ID;
  }  
...
void mapProcess::createPathBetweenObs(){
  auto t = ros::Time::now();
  ...
  // 在border_list_中，体现相邻obstacle group的border的connection关系
  for (int x=0; x<voronoi.getSizeX(); x++){
    for (int y=0; y<voronoi.getSizeY(); y++){
      if (voronoi.isVoronoi(x,y)){
        auto cell = voronoi.getDataCell(x,y);
  ...
  // 用于后续生成homo_path的connection_graph_的前处理
  // 由于每个border_list的起点是固定的，所以如果candida_connection的两端是跨起点的，会比较难找，这里要对所有border_list包含的所有candidate ID做一个shift，来保证遍历顺序
  // 先求从ID1连到ID2的过程中，ID1上可连点的index：connections_indexs；再求ID连ID2中 顺时针的两个端点index：connection_ends
  // 在ID1的border中，可以连接到ID2的border点的index的集合
  ...
  // {ID1 -> ID2} -> {ID1的start_index, ID1的end_index} (顺时针)
  ...
  //   auto IDs = temp.first;
  //   auto index_pair = temp.second;
  //   std::cout<<IDs.first<<"->"<<IDs.second<<": "<<index_pair.first<<"/"<<border_list_[IDs.first].size()<<" -> "<<index_pair.second<<"/"<<border_list_[IDs.first].size()<<std::endl;
  //   std::cout<<IDs.first<<"->"<<IDs.second<<": ("<<border_list_[IDs.first][index_pair.first].x<<","<<border_list_[IDs.first][index_pair.first].y<<") -> ("<<border_list_[IDs.first][index_pair.second].x<<","<<border_list_[IDs.first][index_pair.second].y<<")"<<std::endl;
  // }


  // 根据starts，在connection_graph_标记clockwise和counterwise的点对 connection_graph_: <ID1->ID2>[connection type] = ID1_border_point -> ID2_border_point
  for (auto temp:connection_ends){
    int ID1 = temp.first.first;
  ...
  // // 遍历所有border的所有candidate_connection，找到dis最小的那个，作为shortest连接
  // 对于一般障碍物间的连接来说，这里一般是用不到的
  for (auto temp:border_list_){
    int ID1 = temp.first;
  ...
  // 求clockwise2counter 和 counterclockwise2clockwise
  for (auto temp:connection_ends){
    ...
    int ID1_index_diff = getMod(ID1_end_index+1+ID1_border_length - ID1_start_index, ID1_border_length);
    // ID2对应的param
    int ID2 = temp.first.second;
    ...
    int ID2_index_diff = getMod(ID2_end_index+1+ID2_border_length - ID2_start_index, ID2_border_length);

    // 考虑clockwise2counterclockwise
    // ROS_INFO("clockwise2counterclockwise start");
    ...
    while(true){
      ...
      // 确定滑动谁
        ...
        if( getMod(ID1_index_temp+1+ID1_border_length - ID1_start_index, ID1_border_length)*1.0 / ID1_index_diff < getMod(ID2_index_temp+1+ID2_border_length - ID2_start_index, ID2_border_length)*1.0 / ID2_index_diff )
    ...
    // ROS_INFO("clockwise2counterclockwise done");

    // 考虑counterclockwise2clockwise
    // ROS_INFO("counterclockwise2clockwise start");
    ...
    // ROS_INFO("counterclockwise2clockwise done");
  }
  ...
  // 寻找start点的连接
  // if need_to_connect_start==false，start在障碍物附近 直接归并连接
  // if need_to_connect_start==true: 找到障碍物到其他borderlist的最短连接
  bool need_to_connect_start = true;
  int attached_x, attached_y;
  // for (int dx=-1; dx<=1 && need_to_connect_start; dx++){
  //   for (int dy=-1; dy<=1 && need_to_connect_start; dy++){
  //     attached_x = start_in_map_x_+dx;
  //     attached_y = start_in_map_y_+dy;
  //     int ID = map_border_labeled_[attached_x][attached_y];
  //     if (ID != 0){
  //       // set connection
  //       need_to_connect_start = false;
  //       connection_graph_[{startID_, map_border_labeled_[attached_x][attached_y]}]["shortest"] = std::make_pair(Point2D(start_in_map_x_, start_in_map_y_, 0, startID_), Point2D(attached_x, attached_y, 0, map_border_labeled_[attached_x][attached_y]));
  //       // set border point's connection to start
  //       for (int j=0; j<border_list_[ID].size(); j++){
  //         if (border_list_[ID][j].x==attached_x && border_list_[ID][j].y==attached_y)
  //           border_list_[ID][j].setStartConnection();
  //       }
  //     }
  //   }
  // }
  attached_x = start_in_map_x_;
  attached_y = start_in_map_y_;
  int ID = map_border_labeled_[attached_x][attached_y];
  if (ID != 0){
    // set connection
    need_to_connect_start = false;
  ...
  if (need_to_connect_start){
    for (auto temp:border_list_){
      int ID = temp.first;
      if (ID==startID_) continue;    //跳过起点
      double min_dis = 99999999999;
      double target_x, target_y;
      int target_index;
      for (int i=0; i<temp.second.size(); i++){
        auto p =temp.second[i];
        std::vector<Point2D> vec_line = Bresenham(Point2D(start_in_map_x_, start_in_map_y_), Point2D(p.x, p.y));
        // if (vec_line.size()<=3) continue;   // 距离太近则直接跳过
        // vec_line.erase(vec_line.begin());
        // vec_line.pop_back();
        bool is_collision = checkCollision(vec_line, ID);
        if (!is_collision){
    ...
    if (temp.first.first==startID_){
      ...
      for (auto connect:connection_graph_){
        ...
        if(connect.first.first==ID_groupA){
          ...
          if (ID_groupB>=startID_) continue;
  ...
  // 寻找goal line的连接，利用每个obs只留一个最近的goal_line的shortest连接
  // 把goal line添加到地图中
  ...
  for (auto temp:border_list_){    
    ...
    for(int i=0; i<temp.second.size(); i++){
      ...
      double min_temp_dis_from_p_obs_to_p_goal = 99999999;
      ...
      for (int goalIndex=goalIndex_start_; goalIndex<label_list_.size(); goalIndex++){
        ...
        bool is_this_goal_line_checked = false;
        for (auto index_temp:index_list_ordered){
          auto p_goal = border_list_[goalID][index_temp];
          // 如果当前的连线比最短连线还长，那它肯定不是shortest，直接break
          double current_dis = hypot(p_obs.x-p_goal.x, p_obs.y-p_goal.y);
          if (is_this_goal_line_checked)
            break;  
          ...
          if (!is_collision){
            ...
            is_this_goal_line_checked = true;
          }
        }
      }
      if(min_temp_dis_from_p_obs_to_p_goal != 99999999)
...
std::vector<std::vector<Eigen::Vector2d>> mapProcess::findHomoPaths(std::vector<std::pair<std::pair<double, double>, std::pair<double, double>>>& goal_endpoints_list){
  ...
  for (auto pathIDs:res){
    // std::cout<<" Current processed path ["<<pathIDs.size()<<"] :";
    // for (auto id:pathIDs)
    //   std::cout<<id<<" ";
    // std::cout<<std::endl;
      ...
      for(int i=shrink_dis_; i<comp.cols-shrink_dis_;i++){
        ...
        if(comp.at<uint8_t>(j,i) && map_obs_labeled_[i-shrink_dis_][j-shrink_dis_]<startID_){
          ...
          if (map_obs_labeled_[i-shrink_dis_][j-shrink_dis_]==0){
            ...
            double min_dis = 999;
            int ID=0;
            for (int dx=-ignore_dis; dx<ignore_dis; dx++){
              for (int dy=-ignore_dis; dy<ignore_dis; dy++){
